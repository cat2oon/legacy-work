...
class Leo(tf.keras.Model):
    ...
    def encode(self, task):
        ...
        # prior = diffs
        prior = tf.concat([task.tr_in3, diffs], axis=1, name='prior')
    ...
    """
        Gaze Model
    """
    def stem_conv(self, num_filters, name):
        return Conv2D(filters=num_filters, name=name, 
    ...
    def _build_gaze_model(self, act='selu'):
        ...
        # support 
        self.sp_norm1 = BatchNormalization()
        self.sp_fc1 = Dense(64, use_bias=False, activation=act, name='support_fc1')
        self.sp_fc2 = Dense(32, use_bias=False, activation=act, name='support_fc2')
        self.sp_fc3 = Dense(32, use_bias=False, activation=act, name='support_fc3')
        
        # shuffle conv common
        sf, nf, lf = 24, 64, 64
        
        # left patch
        self.l_stem_conv = self.stem_conv(sf, 'l_stem_conv')
        self.l_stem_pool = MaxPool2D(padding='SAME', strides=(2,2))
        self.l_stem_act = tf.nn.relu6
        self.l_stem_bn_norm = tfa.layers.InstanceNormalization()
        self.l_stage2_a = SDown(out_channel=32, name='stage_l2')
        self.l_stage2_b = Basic(out_channel=32, name='stage_l2')
        self.l_stage3_a = SDown(out_channel=32, name='stage_l3')
        self.l_stage3_b = Basic(out_channel=32, name='stage_l3')
        self.l_stage4_a = SDown(out_channel=32, name='stage_l4')
        self.l_stage4_b = Basic(out_channel=32, name='stage_l4')
        self.l_fe_gap = GlobalAveragePooling2D()
        
        # right patch
        self.r_stem_conv = self.stem_conv(sf, 'r_stem_conv')
        self.r_stem_pool = MaxPool2D(padding='SAME', strides=(2,2))
        self.r_stem_act = tf.nn.relu6
        self.r_stem_bn_norm = tfa.layers.InstanceNormalization()
        self.r_stage2_a = SDown(out_channel=32, name='stage_r2')
        self.r_stage2_b = Basic(out_channel=32, name='stage_r2')
        self.r_stage3_a = SDown(out_channel=32, name='stage_r3')
        self.r_stage3_b = Basic(out_channel=32, name='stage_r3')
        self.r_stage4_a = SDown(out_channel=32, name='stage_r4')
        self.r_stage4_b = Basic(out_channel=32, name='stage_r4')
        self.r_fe_gap = GlobalAveragePooling2D()        
        
        # eye
        self.eye_fc1 = Dense(64, activation=act, use_bias=False, name='eye_fc1')
        
        # final
        self.final_fc1 = Dense(8, activation='linear', use_bias=True,  name='final_fc1')
        self.final_fc2 = Dense(2, activation='linear', use_bias=False, name='final_fc2')
        
    def extract_left_patch(self, patch):
        x = self.l_stem_conv(patch)
        x = self.l_stem_pool(x)
        x = self.l_stem_act(x)
        x = self.l_stem_bn_norm(x)
        x = self.l_stage2_a(x)
        x = self.l_stage2_b(x)
        x = self.l_stage3_a(x)
        x = self.l_stage3_b(x)
    ...
    def extract_right_patch(self, patch):
        x = self.r_stem_conv(patch)
        x = self.r_stem_pool(x)
        x = self.r_stem_act(x)
        x = self.r_stem_bn_norm(x)
        x = self.r_stage2_a(x)
        x = self.r_stage2_b(x)
        x = self.r_stage3_a(x)
        x = self.r_stage3_b(x)
    ...
    def eye_feature(self, face_l, face_r):
        lex = self.extract_left_patch(face_l)
        rex = self.extract_right_patch(face_r)
        x = Concatenate(name='both_eye')([lex, rex])
        x = self.eye_fc1(x)
        return x
    
    def support_feature(self, support):
        x = self.sp_fc1(support)
        x = self.sp_norm1(x)
        x = tf.nn.selu(x)
        x = self.sp_fc2(x)
        x = self.sp_fc3(x)
    ...
    def get_inputs(self, task, from_train=True):
        if from_train:
            return task.tr_in1, task.tr_in2, task.tr_in3
        return task.val_in1, task.val_in2, task.val_in3

    def predict(self, task):
        face_l, face_r, support = self.get_inputs(task, from_train=True)

        spx = self.support_feature(support)
        efx = self.eye_feature(face_l, face_r)
        
        z = Concatenate(name='bottleneck')([efx, spx])
        z = self.final_fc1(z)
        z = self.final_fc2(z)
        return z
    
    def predict_with(self, task, theta, use_train=True):
        face_l, face_r, support = self.get_inputs(task, use_train)
            
        spx = self.support_feature(support)
        efx = self.eye_feature(face_l, face_r)
        
        z = Concatenate(name='bottleneck')([efx, spx])
        z = self.dense_from(z, theta, 96, 8)
        z = self.final_fc2(z)
        return z
    ...
    def get_trainable_variables(self):
        ...
        # filter_names = ['final_fc1', 'conv2d/kernel:0', 'conv2d/bias:0', 'instance_normalization/gamma:0', 'instance_normalization/beta:0']
        # filter_names = ['conv2d/kernel:0', 'conv2d/bias:0', 'instance_normalization/gamma:0', 'instance_normalization/beta:0']
        # filter_names = ['sp_fc1/kernel:0', 'sp_fc1/bias:0', 'sp_fc2/kernel:0', 'sp_fc2/bias:0', 'sp_fc3/kernel:0', 'sp_fc3/bias:0']
        filter_names = []
