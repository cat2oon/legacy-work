...
class DataProvider():
    ...
    def to_named_tasks(self, train, valid):
        Task = nt("task", [ "tr_in1",  "tr_in2", "tr_in3", "tr_output",
                            "val_in1", "val_in2", "val_in3", "val_output" ])
        tasks = []
        ...
        for ids_in_task in ids_in_batch:
            for seq_id, item_id in enumerate(ids_in_task):
                ...
                for ei, input_val in enumerate(process_item):
                    batch_router[ei].append(input_val)        
    ...
    def processed_item(self, id):
        ...
        face_l = face[:,:128,:]
        face_r = face[:,128:,:]
        face_l = self.preprocess_image(face_l, (128, 128))
        face_r = self.preprocess_image(face_r, (128, 128))
        ...
        # head_pose = np.array(m['head_pose'])
        landmark_3d = np.array(m['landmarks_3d']).flatten()
        support = np.hstack([uv_ec, landmark_3d])
        
        return face_l, face_r, support, target_xy 
