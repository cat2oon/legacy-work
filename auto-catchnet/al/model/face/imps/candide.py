...
def eye_landmarks_indices():
    l_eye_indices = (36, 37, 38, 39, 40, 41)
    r_eye_indices = (42, 43, 44, 45, 46, 47)
    return l_eye_indices, r_eye_indices


def caruncle_landmarks_indices():
    l_caruncle_index = 39
    r_caruncle_index = 42
    return l_caruncle_index, r_caruncle_index


def eye_end_landmarks_indices():
    l_eye_indices = (36, 39)
    r_eye_indices = (42, 45)
    return l_eye_indices, r_eye_indices


def eye_landmarks_for_center_indices():
    l_eye_indices = (37, 38, 40, 41)
    r_eye_indices = (43, 44, 46, 47)
    return l_eye_indices, r_eye_indices
...
class Candide:
    ...
    def get_landmarks(self, img, max_size_for_detect=320):
        landmarks = self.get_facial_points(img, max_size_for_detect)
    ...
    def get_projection_params(self, landmarks):
        blend_shapes = self.blend_shapes
        mean_3d_shape = self.mean_3d_shape
        projection = OrthographicProjectionBlendshapes(blend_shapes.shape[0])

        shape_2ds = landmarks.T[:, self.idxs_2d]
        blend_3ds = blend_shapes[:, :, self.idxs_3d]
        mean_3ds = mean_3d_shape[:, self.idxs_3d]

        data_points = ([mean_3ds, blend_3ds], shape_2ds)
        init_params = projection.get_initial_params(mean_3ds, shape_2ds)
        model_params = gauss_newton(init_params, projection.residual, projection.jacobian, data_points)

        return model_params

    def get_head_pose(self, img):
        landmarks = self.get_landmarks(img)
        return self.landmark_to_head_pose(landmarks)

    def landmark_to_head_pose(self, landmarks):
        params = self.get_projection_params(landmarks)
        pitch_r, yaw_r, roll_r = params[1:4]
        pitch, yaw, roll = rad_to_degree(pitch_r), rad_to_degree(yaw_r), rad_to_degree(roll_r)
        return pitch, yaw, roll

    # TODO: 그려서 해보기
    def get_3d_pose(self, projection_params):
        return self.mean_3d_shape * projection_params
