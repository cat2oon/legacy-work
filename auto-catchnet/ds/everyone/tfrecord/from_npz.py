...
def write_tfexample(item_indexes, writer, block_path):
    item_indexes = filter_item_indexes(item_indexes)
    for item_index in item_indexes:
        try:
            example = to_tfexample(item_index, extract_feature=True)
            if example is None:
                continue
...
def convert(npz_path, out_path, num_block_per_records=16):
    print("*** convert ***")
...
def filter_item_indexes(item_indexes):
    def filter_item(item: ItemIndex):
        if item.is_pad():
            return False
        return True

    return filter(filter_item, item_indexes)
